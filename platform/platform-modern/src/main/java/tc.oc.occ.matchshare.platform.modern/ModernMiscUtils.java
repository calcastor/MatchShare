    ...
    public void showFakeItems(Plugin plugin, Player viewer, Location location, ItemStack item, int count, Duration duration) {
        ...
        for (int i = 0; i < count; i++) {
            final ItemEntity entity =
                    new ItemEntity(
                            nmsPlayer.getCommandSenderWorld(),
                            location.getX(),
                            location.getY(),
                            location.getZ(),
                            CraftItemStack.asNMSCopy(item));

            entity.setDeltaMovement(MiscUtils.randomEntityVelocity(),
                    MiscUtils.randomEntityVelocity(), MiscUtils.randomEntityVelocity());

            sendPacket(viewer, new ClientboundAddEntityPacket(entity, 2, new BlockPos(location.getBlockX(), location.getBlockY(), location.getBlockZ())));
            sendPacket(
                    viewer, new ClientboundSetEntityDataPacket(entity.getId(), entity.getEntityData().packAll()));
